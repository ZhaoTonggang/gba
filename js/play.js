...
req.onload = () => {
	if (req.status === 200) {
		...
		nzip.loadAsync(req.responseText)
			.then(zip => {
				if (!zip) {
					...
					const zdata = zip.file(gameInfo.i + ".gba");
					if (!zdata) {
						showtext();
					} else {
						zdata.async("blob")
		...
		//监听加载按钮
		showload.onclick = () => {
			let screen = document.getElementById('nes-canvas');
...
let joystick = new Joystick({
	...
	//按下时的回调
	btn_down_fn: (event) => {
		window.addEventListener("keydown", GameBoyAdvanceKeypad.prototype.keyboardHandler(event));
	},
	//释放时的回调
	btn_up_fn: (event) => {
		window.addEventListener("keyup", GameBoyAdvanceKeypad.prototype.keyboardHandler(event));
	},
})
//实例化NES按钮
